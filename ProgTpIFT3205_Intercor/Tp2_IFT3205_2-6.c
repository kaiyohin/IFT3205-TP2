...
int main(int argc,char **argv)
{
    ...
    float** MatriceNewImg=fmatrix_allocate_2d(length*2,width*2);
    ...
    for (int i=0; i<length; i++) {
        for (int j = 0; j<width; j++) {
            if (MatriceNewImg[i+length/2][j+width/2]==0) {
                MatriceNewImg[i+length/2][j+width/2]=MatriceImg1[i][j];
            }
            if (MatriceNewImg[i+length/2+decalLength][j+width/2+decalWidth]==0) {
                MatriceNewImg[i+length/2+decalLength][j+width/2+decalWidth]=MatriceImgGRotate[i][j];
            }
        }
    }
    ...
    //Sauvegarde
    SaveImagePgm(NAME_IMG_OUT,MatriceNewImg,2*length,2*width);
